...
namespace KPCapture.Module
{
    ...
    public static class UDPTable
    {
        ...
        public static IEnumerable<uint> FindProcessId(Model.Packet packet)
        {
            ...
            {
                return GetAllUDPConnections()
                    .Where(x => ((x.LocalAddress.Equals(IPAddress.Parse("0.0.0.0")) || x.LocalAddress.Equals(packet.SourceAddress)) && x.LocalPort == packet.SourcePort) ||
                                ((x.LocalAddress.Equals(IPAddress.Parse("0.0.0.0")) || x.LocalAddress.Equals(packet.DestinationAddress)) && x.LocalPort == packet.DestinationPort))
                    .Select(x => x.ProcessId);
            }
            else
            {
                return GetAllUDPv6Connections()
                    .Where(x => (x.LocalAddress.Equals(packet.SourceAddress) && x.LocalPort == packet.SourcePort) ||
                                (x.LocalAddress.Equals(packet.DestinationAddress) && x.LocalPort == packet.DestinationPort))
                    .Select(x => x.ProcessId);
            }
        }
